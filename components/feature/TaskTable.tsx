...
const TaskTable: React.FC<TaskTableProps> = ({ tasks, loading, onTaskClick, onEditClick, onDeleteClick, currentUser }) => {
  const recentlyUpdatedTaskIds = useAppSelector((state) => state.notification.recentlyUpdatedTaskIds);
  ...
  const isRecentlyUpdated = (taskId: string) => recentlyUpdatedTaskIds.includes(taskId);
          ...
          <table className="w-full border-collapse text-left text-sm">
            ...
            <tbody className="divide-y divide-[#e5e8ef]">
              {loading ? (
                ...
                tasks.map((task) => {
                  const highlighted = isRecentlyUpdated(task.id);
                  return (
                    <tr
                      key={task.id}
            ...
            {tasks.map((task) => {
              ...
              return (
                <div
                  key={task.id}
                  onClick={() => onTaskClick?.(task)}
                  className={`bg-white border hover:border-zinc-300 hover:shadow-md transition-all p-5 rounded-2xl shadow-[0_1px_2px_rgba(10,21,48,0.02)] flex flex-col justify-between min-h-[160px] cursor-pointer ${
                    highlighted
                      ? "task-highlight-card border-[#2957ff]/30"
                      : "border-[#e5e8ef]"
                  }`}
                >
                  <div>
